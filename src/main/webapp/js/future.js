 ...
 * ]
 */
future.select = function (id,data) {
    var options = {};
    options.data = data;
    var defaultOptions = {
        language: "zh-CN",
 ...
 */
future.datetimepicker = function (id,options) {
    var nowDate;
    ...
    $(id).datetimepicker(defaultOptions).on('changeDate',function (ev) {
        ...
        var dateVal = future.datePickerFormat(id);
 ...
 * @param date
 */
future.datePickerUpdate = function (id,date) {
    $(id).find('.form-control').val(date);
    $(id).datetimepicker('update');
    return future.datePickerFormat(id);
};
...
future.datePickerFormat = function (id) {
    ...
    if (dateFormat) {
        ...
        var dateId = $(id).attr('data-link-field');
        if (dateId) {
            $(id).find('.form-control').val(future.formatDate(dateValue, dateFormat));
            $(id).find('.form-control').attr('value', future.formatDate(dateValue, dateFormat));
        }
    }
    return $('#' + dateId).val();
};
 ...
 *  @return date yyyy-MM-dd(当前日期)
 */
future.monthPicker = function (id, options) {
    var defaultOptions = {
        startView: 3,
        minView: 3,
        todayBtn: false,
    ...
    $.extend(defaultOptions,options);
    return future.datetimepicker(id,defaultOptions);
};
 ...
 * @returns {*}
 */
future.monthPickerPre = function (id, date) {
    date = future.preMonth(date);
    $(id+' input').val(date);
    $(id).datetimepicker('update');
    $(id+' input').val(future.formatDate(date,"yyyy年mm月"));
    return date;
};
 ...
 * @returns {*}
 */
future.monthPickerNext = function (id, date) {
    date = future.nextMonth(date);
    $(id+' input').val(date);
    $(id).datetimepicker('update');
    $(id+' input').val(future.formatDate(date,"yyyy年mm月"));
    return date;
 ...
 * @return {string}
 */
future.nextMonth = function (date) {
    var arr = date.split('-');
    var year = arr[0]; //获取当前日期的年份
    var month = arr[1]; //获取当前日期的月份
    var day = arr[2]; //获取当前日期的日
 ...
 * @return
 */
future.formatTimestamp = function (timestamp,dateFormat) {
 ...
 * @return {string|XML}
 */
future.formatDate = function (dateValue, dateFormat) {
    if (dateFormat){
 ...
 * @returns yyyy:mm:dd
 */
future.formatTime = function(date){
 ...
 * @return {XML|string}
 */
future.formatDbDate = function (dateValue, dateFormat) {
    var year = dateValue.substring(0,4);
 ...
 * }
 */
future.table = function (id, options) {
    var defaultOptions = {
        ...
        responseHandler:function (result) {
            future.respHandle(result);
            return result.data;
        },
        onLoadSuccess:function () {
            if(options.permission == undefined){
                future.refreshPermission();
            }
    ...
    options.url = future.apiRoot()+options.url;
    $.extend(defaultOptions,options);
    var $table = $(id);
    ...
    if (defaultOptions.height){
        ...
        $table.parent().parent().css('height',future.tableHeight());
        $(window).resize(function() {
            $table.parent().parent().css('height',future.tableHeight());
        });
    }
 ...
 * @param id
 */
future.tableRefresh = function (id) {
 ...
 * }
 */
future.tableQuery = function (id,params) {
    $(id).bootstrapTable('refresh', {
        query:params
    });
};
 ...
 * @param newUrl
 */
future.tableQueryUrl = function (id,newUrl) {
    $(id).bootstrapTable('refresh', {
        url:future.apiRoot()+ newUrl
    });
};
 ...
 * @return {*}
 */
future.tableRow = function (id,index) {
 ...
 * @param handler 回调函数
 */
future.openDialog= function (options,handler) {
    var defaultOptions = {
        title:"标题",
        width:600,
        height:300,
 ...
 */
future.submitForm = function (options, successHandler, beforeHandler, failHandler) {
    ...
    //打开弹窗
    future.openDialog(options, function (dialogId) {
 ...
 */
future.alert = function (options, handler) {
    ...
    $(dialogId).on('hidden.future.modal', function (e) {
        $(dialogId).remove();
    });
 ...
 * @return {string|*}
 */
future.apiRoot = function () {
    if(future.rootUrl){
        return future.rootUrl;
    }
    var js=document.scripts;
    var jsroot='';
    for(var i=js.length;i>0;i--){
        jsroot=js[i-1].src;
        if(jsroot.indexOf("/admin/js/common")>-1){
            jsroot=jsroot.substring(0,jsroot.indexOf("/admin/js/common"));
            break;
        }
    }
    future.rootUrl = jsroot;
    return future.rootUrl;
};
...
future.ajax = function (option) {
    $.ajax({
        ...
        url : future.apiRoot()+option.url,
        dataType : 'json',
        timeout : TIME_OUT,
        data:option.data,
        async:option.async==null?true:option.async,
        success : function(result) {
            future.respHandle(result,option);
        },
        error : function(xhr, type) {
            future.toast('error',xhr.status,xhr.statusText);
        }
    });
};
 ...
 * @param option 处理方法
 */
future.respHandle = function (result, option) {
    if (result.code == null){
        future.toast('响应格式不正确！');
        return;
    }
    if (result.code == 'ok'){
        //成功
 ...
 * @param result
 */
future.errorMsg = function(result){
    future.toast('error',result.businessCode,result.businessNote);
};
 ...
 * @param msg 消息
 */
future.toast = function (type,title,msg) {
    // toast-top-right
    // toast-bottom-right
    // toash-bottom-left
    // toast-top-left
 ...
 * 刷新权限
 */
future.refreshPermission = function () {
 ...
 * @return {*}
 */
future.getUrlParameter = function (name) {
 ...
 * @returns {*}
 */
future.formateDateOrTime = function (value,type,splitParam) {
    if(splitParam)
        if(type=='date')
 ...
 */
future.moveDays=function(date,offset,dateFormat){
    var newDate = date;
 ...
 * @returns {*}
 */
future.addArrFull=function (arr,id,text) {
    id=(id==null?"null":id);
 ...
 * return
 */
future.maskCode =function (str, beginNum, endNum){
    if(str != null && str.length > beginNum && str.length > endNum){
        var b = str.substring(0,beginNum);
        var e = str.substring(str.length-endNum,str.length);
        var mask = "";
        for(var i=0;i<(str.length-beginNum-endNum);i++){
            mask +="*";
        }
        return b+mask+e;
    }
